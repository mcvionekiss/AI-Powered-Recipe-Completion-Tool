...
router.delete("/data", async (req, res) => {
  try {
    ...
    await db.execute(
      "DELETE FROM recipe WHERE id=? and userID=6",
      [data[0][0].id],
      [data[0][0].userId]
    );
...
router.post("/addRecipe", async (req, res) => {
  ...
  const { name, description, instructions, foodCategory, cuisine } = req.body;
  ...
  try {
    await db.execute(
      "INSERT INTO recipe (name, description, instructions, foodCategory, cuisine, userId) VALUES (?, ?, ?, ?, ?, ?)",
      [name, description, instructions, foodCategory, cuisine, 6]
    );
